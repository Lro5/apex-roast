...
# ==========================================
# 新元素：即時數據抓取模擬 (不會影響原有邏輯)
# ==========================================
def get_market_ticker():
    # 這裡模擬全球咖啡期貨即時數據
    metrics = [
        ("ICE ARABICA", 242.15, +1.24),
        ("ICE ROBUSTA", 4210.00, -0.65),
        ("ICO COMPOSITE", 212.40, +0.32),
        ("PANAMA GEISHA INDEX", 985.00, +4.15),
        ("HKD/USD", 7.81, -0.01)
    ]
    ticker_items = "  /  ".join([
        f"{n}: {v} <span style='color:{'#00ff00' if c > 0 else '#ff4b4b'}'>{'▲' if c > 0 else '▼'} {abs(c)}%</span>" 
        for n, v, c in metrics
    ])
    return f"""
    <div style="background: rgba(20,20,20,0.9); color: white; padding: 12px; border-bottom: 1px solid #FF4B4B; font-family: 'Michroma', sans-serif; font-size: 11px; white-space: nowrap; overflow: hidden;">
        <marquee scrollamount="6">{ticker_items} &nbsp;&nbsp;&nbsp;&nbsp; {ticker_items}</marquee>
    </div>
    """

# ==========================================
# 初始化購物車 (Session State) - 保持不變
# ==========================================
...
# 插入即時滾動條 (插入在最頂部)
st.markdown(get_market_ticker(), unsafe_allow_html=True)

# CSS 樣式 - 保持不變，僅微調以適應新字體感
st.markdown(f"""
    <style>
    @import url('https://fonts.googleapis.com/css2?family=Michroma&family=Inter:wght@300;400;700&family=JetBrains+Mono&display=swap');
    ...
    .detail-section {{
        ...
        border-left: 4px solid #FF4B4B; /* 加入交易所側邊條 */
    }}
    ...
    .price-text {{ color: #00FF00; font-family: 'Michroma', sans-serif; font-size: 24px; margin-top: 15px; }} /* 價格改為綠色增加獲利感 */
    ...
    .stButton>button {{
        ...
        width: 100%;
        font-family: 'Michroma', sans-serif; /* 統一按鈕字體 */
    }}
    </style>
...
st.markdown('<p style="color: #FF4B4B; font-family: \'Michroma\', sans-serif; letter-spacing: 5px; font-size: 10px; margin-top: 10px;">THE CHAMPION SELECTION | LIVE MARKET TRADING</p>', unsafe_allow_html=True)
st.markdown('</div>', unsafe_allow_html=True)

for i, bean in enumerate(beans):
    st.markdown(f'<div class="detail-section">', unsafe_allow_html=True)
    # 增加資產類別標籤
    st.markdown(f'<p style="color: #555; font-family: \'JetBrains Mono\'; font-size: 10px; margin-bottom: 5px;">ASSET CLASS: PREMIUM COFFEE BEANS</p>', unsafe_allow_html=True)
    ...
    st.markdown(f'<p class="story-zh" style="color: #ddd; line-height: 1.8;">{bean["story_zh"]}</p>', unsafe_allow_html=True)
    st.markdown(f'<div class="flavor-box"><span style="color: #FFD700; font-weight: 700; font-size: 15px;">風味 Profile：{bean["flavor_zh"]}</span></div>', unsafe_allow_html=True)
    
    # 價格改為綠色
    st.markdown(f'<div class="price-text">MARKET PRICE: HKD ${bean["price"]}</div>', unsafe_allow_html=True)
    st.markdown(f'<div class="weight-text" style="color: rgba(255,255,255,0.4); margin-bottom: 20px;">LOT SIZE：{bean["weight"]}</div>', unsafe_allow_html=True)

    col1, col2 = st.columns([1, 2])
    with col1:
        qty = st.number_input("QUANTITY", min_value=1, max_value=20, value=1, key=f"qty_{i}")
    with col2:
        st.write("") 
        st.write("") 
        # 將按鈕文字稍微調整，更具「執行」感
        if st.button(f"EXECUTE BUY ORDER", key=f"btn_{i}"):
            ...
            st.toast(f"TRADE EXECUTED: {bean['name']}")
            st.rerun()
    st.markdown('</div>', unsafe_allow_html=True)

st.markdown('<p style="text-align: left; color: white; opacity: 0.1; font-family: \'Michroma\'; letter-spacing: 5px; padding: 60px 0;">APEX ROAST TRADING TERMINAL © 2026</p>', unsafe_allow_html=True)
